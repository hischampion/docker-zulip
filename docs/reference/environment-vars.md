...
The name of the remote database; defaults to `zulip`.

### `DB_USER`

The username to user when connecting to the remote database; defaults to `zulip`.
...
All `SETTING_` and `SECRET_` environment keys are ignored, and
`/etc/zulip/settings.py` and `/etc/zulip/zulip-secrets.conf` in the container
are the authoritive sources of the configuration. This is generally useful if
...
The number of `nginx` worker processes to run; defaults to 2. Larger
deployments should increase this.

### `PROXY_ALLOW_ADDRESSES`

This option is the equivalent of
[`http_proxy.allow_addresses`](inv:zulip:*#production/system-configuration); a
comma-separated list of private IP addresses which the outgoing HTTP proxy will
allow connections to.

:::{seealso}

- {doc}`/how-to/compose-outgoing-proxy`
...
This option is the equivalent of
[`http_proxy.allow_ranges`](inv:zulip:*#production/system-configuration); a
comma-separated list of CIDRs in private IP space which the outgoing HTTP proxy
