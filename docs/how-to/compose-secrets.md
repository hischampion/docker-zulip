...
services, as well as within Zulip itself. These secrets are used to
authenticate connections between services in the deployment, serving as defense
in depth from
[SSRF](https://owasp.org/www-community/attacks/Server_Side_Request_Forgery), as
well as to authenticate to outside providers (e.g. outgoing email services).
...
for a complete reference on the syntax.
...
`secrets` attibute for the `zulip` service.
...
# Additional secret
ZULIP__GIPHY_API_KEY=PS42baKkLnOUBOqb1BgTyna87ooKgthE
```
